...
function searchArtists(searchTerm) {
    ...
    artistCards.forEach(card => {
        ...
        const artistGenre = card.querySelector('.artist-genre') ? 
            card.querySelector('.artist-genre').textContent.toLowerCase() : '';
        
        const matches = artistName.includes(searchTerm) || artistGenre.includes(searchTerm);
...
function openBookingModal(artistName, artistId) {
...
function updateBookingSummary() {
    ...
    let estimatedPrice = selectedArtist.basePrice || 15000;
...
function viewArtistProfile(artistId) {
    window.location.href = `/artistas/perfil/${artistId}`;
}
